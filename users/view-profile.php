<?php if($user->get_user_access($user_id) != "Supervisor" && $id != $user_id){
  ...
  <form method="POST" action="processes/process.user.php?action=update">
        ...
        <div class="user-id">
          ...
          <div class ="block1" id="block1-bottom">
            ...
            <div class="sub-block">
              <label class="access" for="access">User Access</label>
              <input type="text" name="access" disabled value="<?php echo $user->get_user_access($id);?>" >
            </div>
          </div>
        </div>  
  </form>
